...
public class ClassFrameIRIGeneralisation {
    ...
    public ClassFrameIRIGeneralisation(ClassFrame frame){
    ...
    public boolean isIsomorphic(ClassFrameIRIGeneralisation f){
        if(f.size() != this.size)
            return false;
    ...
    private boolean isIsomorphic(TreeMap<Integer,Set<SyntaxTree>> here, 
        ...
        for(Map.Entry<Integer,Set<SyntaxTree>> entry : here.entrySet()){
                ...
                if(!there.containsKey(weight)){
                    ...
                    if(!SetIRIGeneralisation.exists(trees,there.get(weight)))
                        return false;
                }
        }
        return true;
    ...
    private void initMap(Map<SyntaxTree,Integer> map, Set<OWLAxiom> set){
        for(OWLAxiom a : set){
            ...
            for(Map.Entry<SyntaxTree,Integer> entry : map.entrySet()){
                ...
                if(IRIGeneralisation.exists(t,insert)){//ATTENTION
                    found = true;
