...
public class ClassFrameCEPreservation {
    ...
    public ClassFrameCEPreservation(ClassFrame frame){
        this.treeBuilder = new CEPreservanceBuilder();//full syntax tree builder
    ...
    public boolean isIsomorphic(ClassFrameCEPreservation f){
        if(f.size() != this.size)
            return false;
        ...
        if(!isIsomorphic(this.weight2superclasses, f.getWeight2superclasses()))
            return false;
        if(!isIsomorphic(this.weight2equivalences, f.getWeight2equivalences()))
            return false;
        if(!isIsomorphic(this.weight2disjointUnion, f.getWeight2disjointUnion()))
            return false;
        if(!isIsomorphic(this.weight2disjointClasses, f.getWeight2disjointClasses()))
            return false; 

        return true;
    }

    private boolean isIsomorphic(TreeMap<Integer,Set<SyntaxTree>> here, 
            TreeMap<Integer,Set<SyntaxTree>> there){

        for(Map.Entry<Integer,Set<SyntaxTree>> entry : here.entrySet()){

                int weight = entry.getKey();
                Set<SyntaxTree> trees = entry.getValue();

                if(!there.containsKey(weight)){
                    return false;
                } else { 
                    if(!SetClassExpressionPreservance.exists(trees,there.get(weight)))
                        return false;
                }
        }
        return true;
    ...
    private void initMap(Map<SyntaxTree,Integer> map, Set<OWLAxiom> set){
        for(OWLAxiom a : set){
            ...
            for(Map.Entry<SyntaxTree,Integer> entry : map.entrySet()){
                ...
                if(ClassExpressionPreservance.exists(t,insert)){//ATTENTION
                    found = true;
